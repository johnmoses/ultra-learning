...
export default function StudyPack() {
  ...
  const fetchFlashcards = async () => {
    try {
      const response = await api.get(`/api/learning/flashcards?pack_id=${packId}`);
  ...
  const handleDifficultyRating = async (difficulty: 'easy' | 'medium' | 'hard') => {
    ...
    try {
      await api.post('/api/engagement/add-points', { points });
    ...
    if (currentIndex === flashcards.length - 1) {
      ...
      try {
        await api.post('/api/dashboard/sessions', {
          subject: 'Flashcards',
